...
- `DELETE /api/instances/{name}` → 停止实例（校验名 → 读 instance.json pid → `taskkill /F`，进程已死视为已停）并打 `stopped:true` 标记（返回 `{ok,name,stopped:pid}`）——手工停止语义：主实例启动的自动复活跳过之；数据目录保留，同工作区 POST 复活换新端口（重写 meta 清除标记）；主实例不注册于 `.instances/`（无实例目录），故不可按名删除
- **启动自动复活（W17 迭代九）**：主实例（web 前端 + 交互模式 + 非子实例，三重闸门）启动时后台扫 `.instances/`，对**未手工停止且已死**的注册实例按原目录/工作区重新 spawn——换新端口，memory/config/会话延续；meta 回写新 pid/port（`created_at` 保留）。串行 spawn + 逐个就绪等待（防端口竞态/启动风暴）；复活不改 `.last-workspace`。子进程独立于父存活（杀主不杀子），仍在运行的实例身份探活命中即跳过。`start-window.cmd` 直启的实例不注册 instance.json（手工生命周期），不参与自动复活
