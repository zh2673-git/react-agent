...
- `PUT /api/config` → 分段合并落盘 + 热应用：`llm` 逐字段（null 不覆盖，key 热应用 env）；`tools.enabled` 白名单整体替换（configure 热生效）；`agent` 逐字段（null 不覆盖）；`media` 逐字段校验落盘（tools PLAN §九：image 组 base_url/model/key，video 组 base_url/model/key/submit_url/query_url + `extra` 站点默认参数（W19：须为对象——submit 请求体默认值 + poll 查询参数，agent 显式参数优先覆盖，如 Agnes 的 mode/size/model_name）——url 宽松形态校验，null/空串不覆盖，仅持久通道：guest spawn 时 env 固化，**改后需重启 host**）；`mcp_servers` 按 server 名合并——command/cwd 未传保留原值，env 逐键合并（空串=不动），**仅落盘**（server 子进程生命周期归插件 init/destroy，改后需重启 host）
...
- `GET /api/instances` → 实例清单 `[{name,pid,port,url,workspace,created_at,online,stopped?}]`（online=**身份探活**：端口连通且该端口 `/api/config` 回显的 `instance_name` 与实例名一致——仅测 TCP 会「探到别人家」：重启后端口被其他实例/进程复用时假报在线；`.instances/` 扫描）
- `DELETE /api/instances/{name}` → 停止实例（校验名 → 读 instance.json pid → `taskkill /F`，进程已死视为已停）并打 `stopped:true` 标记（返回 `{ok,name,stopped:pid}`）——手工停止语义：实例列表标注「已手动停止」；数据目录保留，同工作区 POST 复活换新端口（重写 meta 清除标记）；主实例不注册于 `.instances/`（无实例目录），故不可按名删除
- **按需复活（W17 迭代十，用户决策「打开哪个恢复哪个」）**：主实例启动**不再批量拉起**注册实例（迭代九的 `revive_instances` 已删除）；恢复入口 =「新窗口」modal 实例列表（`GET /api/instances`）——在线实例点「打开」直达，离线实例点「启动」按原工作区 POST 复活（同工作区复活路径：换新端口，memory/config/会话延续，换新端口后 meta 重写）。探活并行化（`thread::scope` + connect 500ms 超时）：多个离线实例的清单加载 ≈ 最慢一个而非线性求和。复活不改 `.last-workspace`；`start-window.cmd` 直启的实例不注册 instance.json（手工生命周期），不在列表中
