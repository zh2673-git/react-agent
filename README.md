...
> ⚙ 设置面板：LLM（模型/站点/key + 生图/生视频媒体模型三区独立配置）· 工具 · 技能（自建技能 gongwen-format 及配套工具启停区）· Agent（max_rounds/上下文窗口热通道）。不清楚某站怎么配？直接在对话里问（如「XX 站怎么配」），agent 会调研该站点文档并给出可粘贴的配置（SYSTEM.md「配置协助」纪律；已知站点范例沉淀于 media-gen 技能）。
